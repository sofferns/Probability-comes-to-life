...
void toPlay(){
  ...
  if(mousePressed && mouseButton == LEFT && mouseX > width/2 - 120 && mouseX < width/2 + 120 && mouseY > 268 && mouseY < 362){
   chosen = Games[0];
   chosen.again();
   this.Play = 0;
  }
  if(mousePressed && mouseButton == LEFT && mouseX > width/2 - 120 && mouseX < width/2 + 120 && mouseY > 368 && mouseY < 462){
chosen = Games[1];
chosen.again();
this.Play = 1;
}
